...
def scraping_asignaturas():
    ...
    for url in url_list:
        source = urllib.request.urlopen(
            "https://www.us.es/estudiar/que-estudiar/oferta-de-grados/" + url).read()
        soup = bs.BeautifulSoup(source, "lxml")
        asignaturas = soup.find(
            "table", class_="table-condensed").find("tbody").find_all("tr")
        asignatura_name = soup.find(
            "h1", class_="text-center noticia grado").getText()
        for asignatura in asignaturas:
            curso = str.strip(asignatura.find(
                "td", class_="views-field-field-cur-numcur").getText())
            title = str.strip(asignatura.find(
                "td", class_="views-field views-field-title").find("a").getText())
            if title != "Trabajo Fin de Grado":
                json_fixture = json_fixture + '{ "model": "app.asignatura", "pk":' + str(
                    id_poblacion) + ', "fields": {"nombre": "' + title + '", "titulacion": "' + asignatura_name.strip() + '", "anyo": ' + curso + '} },'
